...
#error All UserPersistentObjects files must be compiled with Non-ARC. Use -fno-objc-arc flag.
#endif
...
@interface UserPersistentObject (private)
...
+(UserPersistentObject*)_findFirstByCriteria:(NSString *)criteriaString, ...;
+(UserPersistentObject*)_findByPK:(int)inPk;
@end
@interface UserPersistentObject (private_memory)
+ (NSString *)memoryMapKeyForObject:(NSInteger)thePK;
+ (void)registerObjectInMemory:(UserPersistentObject *)theObject;
+ (void)unregisterObject:(UserPersistentObject *)theObject;
- (NSString *)memoryMapKey;
@end

// Eric 2016-111-19
NSMutableDictionary *user_objectMap;
NSMutableArray *user_checkedTables;
static NSMutableDictionary* _knownTables;

@implementation UserPersistentObject
...
+ (double)performSQLAggregation: (NSString *)query, ...
{
    ...
    [[UserDBManager sharedManager] performUsingDBOperationQueue:^{
        sqlite3 *database = [[UserDBManager sharedManager] database];
...
+ (void)clearCache
{
    if(user_objectMap != nil)
        [user_objectMap removeAllObjects];
    
    if(user_checkedTables != nil)
        [user_checkedTables removeAllObjects];
...
+(UserPersistentObject *)findFirstByCriteria:(NSString *)criteriaString, ...
{
    __block UserPersistentObject* result = nil;
...
+(void)deleteObject:(NSInteger)inPk cascade:(BOOL)cascade
{
    ...
    UserPersistentObject* objToDelete = [self findByPK:(int)inPk];
    if(objToDelete == nil)
        return;
...
+(UserPersistentObject *)findByPK:(int)inPk
{
    __block UserPersistentObject* result = nil;
...
+(NSArray *)_findByCriteria:(NSString *)criteriaString, ...
{
    ...
    if (sqlite3_prepare_v2( database, [query UTF8String], -1, &statement, NULL) == SQLITE_OK)
    {
        while (sqlite3_step(statement) == SQLITE_ROW)
        {
            ...
            id oneItem = [user_objectMap objectForKey:memoryMapKey];
            if (oneItem)
            {
            ...
            for (i=0; i <  column_count; i++)
            {
                ...
                else
                {
                    ...
                    else if ([colType hasPrefix:@"@"])
                    {
                        ...
                        if ([propClass isSubclassOfClass:[UserPersistentObject class]])
                        {
...
+ (void)performUsingDBOperationQueue:(void(^)(void))block
{
    ...
    dispatch_retain([UserPersistentObject DBOperationQueue]);
    dispatch_sync([UserPersistentObject DBOperationQueue], block);
    dispatch_release([UserPersistentObject DBOperationQueue]);
}
...
- (void)_save
{
    ...
    if (!dirty)
    {
        ...
        for (NSString *propName in props)
        {
            ...
            if ([propType hasPrefix:@"@"] ) // Object
            {
                ...
                if (! (isCollectionType(className)) )
                {
                    if ([[theProperty class] isSubclassOfClass:[UserPersistentObject class]])
                ...
                else
                {
                    if (isNSSetType(className) || isNSArrayType(className))
                    {
                        ...
                        {
                            if ([oneObject isKindOfClass:[UserPersistentObject class]] && [oneObject isDirty])
                                dirty = YES;
                        }
                    }
                    else if (isNSDictionaryType(className))
    ...
    if (dirty)
    {
                ...
                else if ([propType hasPrefix:@"@"] ) // Object
                {
                    ...
                    else
                    {
                        ...
                        if (isNSArrayType(className))
                        {
                            ...
                            for (id oneObject in (NSArray *)theProperty)
                            {
                                if ([oneObject isKindOfClass:[UserPersistentObject class]])
                                {
                                    [oneObject _save];
                        ...
                        else if (isNSDictionaryType(className))
                        {
                            for (NSString *oneKey in (NSDictionary *)theProperty)
                            {
                                ...
                                if ([(NSObject *)oneObject isKindOfClass:[UserPersistentObject class]])
                                {
                                    [(UserPersistentObject *)oneObject _save];
                                    NSString *xrefInsert = [NSString stringWithFormat:@"insert into %@_%@ (parent_pk, dictionary_key, fk, fk_table_name) values (%ld, '%@', %d, '%@')",  [[self class] tableName], [propName stringAsSQLColumnName], (long)pk, oneKey, [(UserPersistentObject *)oneObject pk], [[oneObject class] tableName]];
                                    if (sqlite3_exec (database, [xrefInsert UTF8String], NULL, NULL, &errmsg) != SQLITE_OK)
                                        NSLog(@"Error inserting child rows in xref table for array: %s", errmsg);
                                    sqlite3_free(errmsg);
                                }
        ...
        if (![[user_objectMap allKeys] containsObject:[self memoryMapKey]])
...
-(void)revert
{
    ...
    UserPersistentObject* dbObj = [[self class] findByPK:[self pk]];
        ...
        if ([colType hasPrefix:@"@"])
        {
            ...
            if (isCollectionType(className))
            {
                if (cascade)
                {
                    ...
                    //					// a subclass of UserPersistentObject
            ...
            {
                Class propClass = objc_lookUpClass([className UTF8String]);
                if ([propClass isSubclassOfClass:[UserPersistentObject class]] && cascade)
                {
...
+(void)tableCheck
{
    ...
    if (user_checkedTables == nil)
        user_checkedTables = [[NSMutableArray alloc] init];
    
    if (![user_checkedTables containsObject:[self className]])
    {
        [user_checkedTables addObject:[self className]];
        ...
        sqlite3 *database = [[UserDBManager sharedManager] database];
        if (database == NULL) {
            [user_checkedTables removeObject: [self className]];
            return;
        }
        ...
        for (NSString *oneProp in props)
        {
            ...
            else if ([propType hasPrefix:@"@"] ) // Object
            {
                ...
                // object stored in the collection is a subclass of UserPersistentObject, then it is stored as
                ...
                // UserPersistentObject and doesn't conform to NSCoding then the object won't get persisted.
                if (isNSArrayType(className))
                {
            ...
            if (![tableCols containsObject:propName])
            {
                ...
                if ([propType hasPrefix:@"@"])
                {
                    ...
                    else
                    {
                        ...
                        [[UserDBManager sharedManager] executeUpdateSQL:[NSString stringWithFormat:@"alter table %@ add column %@ %@", [self tableName], propName, colType]];
                    }
                }
                else
                {
...
{
    if (user_objectMap == nil)
        user_objectMap = [[NSMutableDictionary alloc] init];
    
    [user_objectMap setObject:theObject forKey:[theObject memoryMapKey]];
...
+ (void)unregisterObject:(UserPersistentObject *)theObject
{
    ...
    UserPersistentObject *compare = [user_objectMap objectForKey:[theObject memoryMapKey]];
    if (compare == theObject)
        [user_objectMap removeObjectForKey:[theObject memoryMapKey]];
}

+ (NSMutableDictionary *)getCacheTableDic
{
    return user_objectMap;
}

- (void)removeTableCache:(UserPersistentObject *)persistentObject
{
    if (persistentObject == nil) {
        return;
    }
    NSString *keyString = [[NSString alloc] initWithFormat: @"%@-%d", [persistentObject className], persistentObject.pk];
    [user_objectMap removeObjectForKey: keyString];
    [keyString release];
}
