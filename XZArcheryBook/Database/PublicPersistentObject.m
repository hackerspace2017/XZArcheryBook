...
#error All PublicPersistentObjects files must be compiled with Non-ARC. Use -fno-objc-arc flag.
#endif
...
@interface PublicPersistentObject (private)
...
+(PublicPersistentObject*)_findFirstByCriteria:(NSString *)criteriaString, ...;
+(PublicPersistentObject*)_findByPK:(int)inPk;
@end
@interface PublicPersistentObject (private_memory)
+ (NSString *)memoryMapKeyForObject:(NSInteger)thePK;
+ (void)registerObjectInMemory:(PublicPersistentObject *)theObject;
+ (void)unregisterObject:(PublicPersistentObject *)theObject;
- (NSString *)memoryMapKey;
@end

// Eric 2016-111-19
NSMutableDictionary *public_objectMap;
NSMutableArray *public_checkedTables;
static NSMutableDictionary* _knownTables;

@implementation PublicPersistentObject
...
+ (double)performSQLAggregation: (NSString *)query, ...
{
    ...
    [[PublicDBManager sharedManager] performUsingDBOperationQueue:^{
        sqlite3 *database = [[PublicDBManager sharedManager] database];
...
+ (void)clearCache
{
    if(public_objectMap != nil)
        [public_objectMap removeAllObjects];
    
    if(public_checkedTables != nil)
        [public_checkedTables removeAllObjects];
...
+(PublicPersistentObject *)findFirstByCriteria:(NSString *)criteriaString, ...
{
    __block PublicPersistentObject* result = nil;
...
+(void)deleteObject:(NSInteger)inPk cascade:(BOOL)cascade
{
    ...
    PublicPersistentObject* objToDelete = [self findByPK:(int)inPk];
    if(objToDelete == nil)
        return;
...
+(PublicPersistentObject *)findByPK:(int)inPk
{
    __block PublicPersistentObject* result = nil;
...
+(NSArray *)_findByCriteria:(NSString *)criteriaString, ...
{
    ...
    if (sqlite3_prepare_v2( database, [query UTF8String], -1, &statement, NULL) == SQLITE_OK)
    {
        while (sqlite3_step(statement) == SQLITE_ROW)
        {
            ...
            id oneItem = [public_objectMap objectForKey:memoryMapKey];
            if (oneItem)
            {
            ...
            for (i=0; i <  column_count; i++)
            {
                ...
                else
                {
                    ...
                    else if ([colType hasPrefix:@"@"])
                    {
                        ...
                        if ([propClass isSubclassOfClass:[PublicPersistentObject class]])
                        {
...
+ (void)performUsingDBOperationQueue:(void(^)(void))block
{
    ...
    dispatch_retain([PublicPersistentObject DBOperationQueue]);
    dispatch_sync([PublicPersistentObject DBOperationQueue], block);
    dispatch_release([PublicPersistentObject DBOperationQueue]);
}
...
- (void)_save
{
    ...
    if (!dirty)
    {
        ...
        for (NSString *propName in props)
        {
            ...
            if ([propType hasPrefix:@"@"] ) // Object
            {
                ...
                if (! (isCollectionType(className)) )
                {
                    if ([[theProperty class] isSubclassOfClass:[PublicPersistentObject class]])
                ...
                else
                {
                    if (isNSSetType(className) || isNSArrayType(className))
                    {
                        ...
                        {
                            if ([oneObject isKindOfClass:[PublicPersistentObject class]] && [oneObject isDirty])
                                dirty = YES;
                        }
                    }
                    else if (isNSDictionaryType(className))
    ...
    if (dirty)
    {
                ...
                else if ([propType hasPrefix:@"@"] ) // Object
                {
                    ...
                    else
                    {
                        ...
                        if (isNSArrayType(className))
                        {
                            ...
                            for (id oneObject in (NSArray *)theProperty)
                            {
                                if ([oneObject isKindOfClass:[PublicPersistentObject class]])
                                {
                                    [oneObject _save];
                        ...
                        else if (isNSDictionaryType(className))
                        {
                            for (NSString *oneKey in (NSDictionary *)theProperty)
                            {
                                ...
                                if ([(NSObject *)oneObject isKindOfClass:[PublicPersistentObject class]])
                                {
                                    [(PublicPersistentObject *)oneObject _save];
                                    NSString *xrefInsert = [NSString stringWithFormat:@"insert into %@_%@ (parent_pk, dictionary_key, fk, fk_table_name) values (%ld, '%@', %d, '%@')",  [[self class] tableName], [propName stringAsSQLColumnName], (long)pk, oneKey, [(PublicPersistentObject *)oneObject pk], [[oneObject class] tableName]];
                                    if (sqlite3_exec (database, [xrefInsert UTF8String], NULL, NULL, &errmsg) != SQLITE_OK)
                                        NSLog(@"Error inserting child rows in xref table for array: %s", errmsg);
                                    sqlite3_free(errmsg);
                                }
        ...
        if (![[public_objectMap allKeys] containsObject:[self memoryMapKey]])
...
-(void)revert
{
    ...
    PublicPersistentObject* dbObj = [[self class] findByPK:[self pk]];
        ...
        if ([colType hasPrefix:@"@"])
        {
            ...
            if (isCollectionType(className))
            {
                if (cascade)
                {
                    ...
                    //					// a subclass of PublicPersistentObject
            ...
            {
                Class propClass = objc_lookUpClass([className UTF8String]);
                if ([propClass isSubclassOfClass:[PublicPersistentObject class]] && cascade)
                {
...
+(void)tableCheck
{
    ...
    if (public_checkedTables == nil)
        public_checkedTables = [[NSMutableArray alloc] init];
    
    if (![public_checkedTables containsObject:[self className]])
    {
        [public_checkedTables addObject:[self className]];
        ...
        sqlite3 *database = [[PublicDBManager sharedManager] database];
        if (database == NULL) {
            [public_checkedTables removeObject: [self className]];
            return;
        }
        ...
        for (NSString *oneProp in props)
        {
            ...
            else if ([propType hasPrefix:@"@"] ) // Object
            {
                ...
                // object stored in the collection is a subclass of PublicPersistentObject, then it is stored as
                ...
                // PublicPersistentObject and doesn't conform to NSCoding then the object won't get persisted.
                if (isNSArrayType(className))
                {
            ...
            if (![tableCols containsObject:propName])
            {
                ...
                if ([propType hasPrefix:@"@"])
                {
                    ...
                    else
                    {
                        ...
                        [[PublicDBManager sharedManager] executeUpdateSQL:[NSString stringWithFormat:@"alter table %@ add column %@ %@", [self tableName], propName, colType]];
                    }
                }
                else
                {
...
{
    if (public_objectMap == nil)
        public_objectMap = [[NSMutableDictionary alloc] init];
    
    [public_objectMap setObject:theObject forKey:[theObject memoryMapKey]];
...
+ (void)unregisterObject:(PublicPersistentObject *)theObject
{
    ...
    PublicPersistentObject *compare = [public_objectMap objectForKey:[theObject memoryMapKey]];
    if (compare == theObject)
        [public_objectMap removeObjectForKey:[theObject memoryMapKey]];
}

+ (NSMutableDictionary *)getCacheTableDic
{
    return public_objectMap;
}

- (void)removeTableCache:(PublicPersistentObject *)persistentObject
{
    if (persistentObject == nil) {
        return;
    }
    NSString *keyString = [[NSString alloc] initWithFormat: @"%@-%d", [persistentObject className], persistentObject.pk];
    [public_objectMap removeObjectForKey: keyString];
    [keyString release];
}
